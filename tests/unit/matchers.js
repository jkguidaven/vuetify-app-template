...
customMatchers.toBeAViewComponent = function (options, mockInstance) {
  if (usesALayout() && definesAPageTitleAndDescription()) {
    return {
      message: () => `expected ${this.utils.printReceived(options)} not to register a local Layout component nor define a page title and meta description`,
      pass: true,
    }
  }
  return {
  ...
  function definesAPageTitleAndDescription () {
    ...
    const hasMetaDescription = pageObject.meta.some(metaProperty => metaProperty.name === 'description' &&
...
customMatchers.toBeAVuexModule = function (options) {
  ...
  return {
    message: () => `expected ${this.utils.printReceived(options)} to be a valid Vuex module, include state, getters, mutations, and actions`,
    pass: false,
  }
